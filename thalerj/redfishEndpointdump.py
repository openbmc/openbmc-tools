...
def createCommandParser():
    ...
    parser.add_argument('-f', '--file', help='The path and filename of where to save the API dump')
    return parser
...
#    response = mysess.get('https://{host}{endpoint}'.format(host=host, endpoint=endpoint), headers=headers, verify=False, auth=(user,pw))
    response = mysess.get('https://{host}{endpoint}'.format(host=host, endpoint=endpoint), headers=jsonHeader, verify=False)
...
def login(host, user, pw):
    ...
    response = mysess.post('https://{host}/redfish/v1/SessionService/Sessions'.format(host=host), headers=jsonHeader, data=data, verify=False)
...
#        mysess = requests.session()
        completedEndpoints = []
        apicontents = parseAPI(args.host, args.user, pw, '/redfish/v1', mysess, completedEndpoints)
        if hasattr(args, 'file'):
            with open(args.file, 'w') as f:
                f.write(json.dumps(apicontents, sort_keys=True, indent=4, separators=(',', ': '), ensure_ascii=False))
        else:
            print(json.dumps(apicontents, sort_keys=True, indent=4, separators=(',', ': '), ensure_ascii=False))
