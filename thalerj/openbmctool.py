...
def enableLDAP(host, args, session):
    ...
    try:
        ...
        if (serverTypeEnabled is not None):
            ...
            if (serverTypeToBeEnabled != serverTypeEnabled):
                res = copyRoleMap(host,args,session,serverTypeEnabled,serverTypeToBeEnabled)
                if (res == False):
                    return("Unable to copy the role map from " + serverTypeEnabled + " to " + serverTypeToBeEnabled)
            ...
            if (serverTypeEnabled is not None):
                ...
                for serverType in serverTypeMap.keys():
                    if (serverType != serverTypeEnabled):
                        res = copyRoleMap(host,args,session,serverTypeEnabled,serverType)
                        if (res == False):
                            return("Unable to copy the role map from " + serverTypeEnabled + " to " + serverType)
...
def copyRoleMap(host,args,session,fromServerType,toServerType):

    """
        Called by LDAP related functions to copy the role maps 
    ...
    try:
        ...
        #Check for the existence of role map for the currently enabled server type
        data = {"data": []}
        ...
        #If the group name-privilege mapping exists for the fromServerType, then
        #delete the role maps from the toServerType before copying to avoid duplication
        if (roleMapExists == True):

            #delete the privilege mapping for the newly enabled server type before copying from 
            #the currently enabled server type. This is to avoid duplication of the role maps
            ...
            if (res.status_code != requests.codes.ok):
                print("Unable to read the role map of the Server type " + fromServerType)
                return False
