...
def checkFWactivation(host, args, session):
    ...
    try:
        resp = session.get(url, headers=httpHeader, verify=False, timeout=60)
...
def login(host, username, pw,jsonFormat):
    ...
    try:
        r = mysess.post('https://'+host+'/login', headers=httpHeader, json = {"data": [username, pw]}, verify=False, timeout=60)
...
def logout(host, username, pw, session, jsonFormat):
    ...
    httpHeader = {'Content-Type':'application/json'}
    try:
        r = session.post('https://'+host+'/logout', headers=httpHeader,json = {"data": [username, pw]}, verify=False, timeout=60)
    except(requests.exceptions.Timeout):
...
def sensor(host, args, session):
    ...
    try:
        occres = session.get(url, headers=httpHeader, verify=False, timeout=60)
    except(requests.exceptions.Timeout):
        return(connectionErrHandler(args.json, "Timeout", None))
    if not args.json:
...
def selClear(host, args, session):
    ...
    try:
        res = session.post(url, headers=httpHeader, data=data, verify=False, timeout=60)
    except(requests.exceptions.Timeout):
        return(connectionErrHandler(args.json, "Timeout", None))
    if res.status_code == 200:
        ...
        for key in sels:
            if 'callout' not in key:
                ...
                try:
                    session.post(url, headers=httpHeader, data=data, verify=False, timeout=60)
                except(requests.exceptions.Timeout):
                    return connectionErrHandler(args.json, "Timeout", None)
                    sys.exit(1)
                except(requests.exceptions.ConnectionError) as err:
...
def selSetResolved(host, args, session):
    ...
    data = "{\"data\": 1 }"
    try:
        res = session.put(url, headers=httpHeader, data=data, verify=False, timeout=60)
    except(requests.exceptions.Timeout):
        return(connectionErrHandler(args.json, "Timeout", None))
    if res.status_code == 200:
...
def bmcDumpRetrieve(host, args, session):
    ...
    try:
        r = session.get(url, headers=httpHeader, stream=True, verify=False, timeout=60)
        if (args.dumpSaveLoc is not None):
...
def bmcDumpList(host, args, session):
    ...
    try:
        r = session.get(url, headers=httpHeader, verify=False, timeout=60)
        dumpList = r.json()
        return dumpList
    except(requests.exceptions.Timeout):
        return connectionErrHandler(args.json, "Timeout", None)
...
def bmcDumpDelete(host, args, session):
    ...
    if args.dumpNum is not None:
        ...
        for dumpNum in dumpList:
            ...
            try:
                r = session.post(url, headers=httpHeader, json = {"data": []}, verify=False, timeout=60)
...
def csdLEDs(host,args, session, fileDir):
    ...
    try:
        ...
        for i in range(3):
            try:
                ledRes = session.get(url, headers=httpHeader, verify=False, timeout=60)
...
def csdFullEnumeration(host, args, session, fileDir):
    ...
    try:
        ...
        for i in range(3):
            try:
                bmcRes = session.get(url, headers=httpHeader, verify=False, timeout=180)
