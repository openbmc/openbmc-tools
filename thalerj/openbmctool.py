...
def setPassword(host, args, session):
    """
         ...
         @return: Session object
    """
    url = "https://"+host+"/xyz/openbmc_project/user/"+args.user+"/action/SetPassword"
    httpHeader = {'Content-Type':'application/json'}
    try:
        res = session.post(url, headers=httpHeader, 
                json = {"data": [args.password]}, verify=False, timeout=30)
    except(requests.exceptions.Timeout):
        return(connectionErrHandler(args.json, "Timeout", None))
    return res.text
...
def createCommandParser():
    ...
    parser_set_password =
        subparsers.add_parser("set_password", help="Set password of local user")
