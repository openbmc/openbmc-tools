...
def enableLDAP(host, args, session):
    ...
    if(isLegacySystem):
        res = enableLDAPLegacy(host, args, session)
    else:
        res = enableLDAPLatest(host, args, session)

    return res

def enableLDAPLegacy(host, args, session):
    """
...
def enableLDAPLatest(host, args, session):
    """
...
def identifyLegacySystem(host, session):
...
def main(argv=None):
    ...
    if (hasattr(args, 'fileloc') and args.fileloc is not None and 'print' in args.command):
        ...
        if(hasattr(args, 'host') and hasattr(args,'user')):
            ...
            isLegacySystem = identifyLegacySystem(args.host,mysess)
