...
def bmcReset(host, args, session):
    ...
    if(args.type == "warm"):
        ...
        res = session.put(url, headers=jsonHeader, data=data, verify=False, timeout=20)
        return res.text
    elif(args.type =="cold"):
        print("\nAttempting to reboot the BMC...:")
        url="https://"+host+"/xyz/openbmc_project/state/bmc0/attr/RequestedBMCTransition"
...
def activateFWImage(host, args, session):
    ...
    data1 = "{\"data\": 1 }"
    try:
        resp = session.put(url, headers=jsonHeader, data=data, verify=False, timeout=30)
        resp1 = session.put(url1, headers=jsonHeader, data=data1, verify=False, timeout=30)
    except(requests.exceptions.Timeout):
        return connectionErrHandler(args.json, "Timeout", None)
    except(requests.exceptions.ConnectionError) as err:
        return connectionErrHandler(args.json, "ConnectionError", err)
...
def remoteLogging(host, args, session):
    ...
    try:
        if(args.remote_logging == 'view'):
            ...
            res = session.put(url + '/attr/Port', headers=jsonHeader, json = {"data": 0}, verify=False, timeout=30)
            res = session.put(url + '/attr/Address', headers=jsonHeader, json = {"data": ""}, verify=False, timeout=30)
        else:
...
def remoteLoggingConfig(host, args, session):
    ...
    try:
        res = session.put(url + '/attr/Port', headers=jsonHeader, json = {"data": args.port}, verify=False, timeout=30)
        res = session.put(url + '/attr/Address', headers=jsonHeader, json = {"data": args.address}, verify=False, timeout=30)
    except(requests.exceptions.Timeout):
        return(connectionErrHandler(args.json, "Timeout", None))
    return res.text
...
def enableLDAP(host, args, session):
    ...
    try:
        res = session.post(url, headers=jsonHeader, json=data, verify=False, timeout=30)
    except(requests.exceptions.Timeout):
        return(connectionErrHandler(args.json, "Timeout", None))
    except(requests.exceptions.ConnectionError) as err:
        return connectionErrHandler(args.json, "ConnectionError", err)
...
def listPrivilegeMapping(host, args, session):
    ...
    try:
        res = session.get(url, headers=jsonHeader, json = data, verify=False, timeout=30)
    except(requests.exceptions.Timeout):
        return(connectionErrHandler(args.json, "Timeout", None))
    except(requests.exceptions.ConnectionError) as err:
        return connectionErrHandler(args.json, "ConnectionError", err)
...
def getThermalMode(host, args, session):
    """
        ...
        @param host: string, the hostname or IP address of the bmc
        @param args: contains additional arguments used for to get the thermal
               control mode
        @param session: the active session to use
        @param args.zone: the zone to get the mode on
        @param args.mode: the mode to get
        @return: Session object
    """
    ...
    for prop in propsDict['data']:
        if (prop.casefold() == args.mode.casefold()):
            state = "off"
            if (propsDict['data'][prop]):
                state = "on"
            return args.mode + " = " + state

    return "Thermal control mode(" + args.mode + ") not supported"
...
def setThermalMode(host, args, session):
    """
        ...
        @param args.mode: the mode to set
        @param args.state: the state to set the mode to
        @return: Session object
    """
    url = "https://" + host + "/xyz/openbmc_project/control/thermal/" + \
        args.zone + "/attr/" + args.mode
    data = ''
    if not args.state:
        return getThermalMode(host, args, session)
    else:
        if (args.state == 'on'):
            data = '{"data":true}'
        elif (args.state == 'off'):
            ...
            return "Invalid thermal control mode"
...
def createCommandParser():
    ...
    #get thermal control mode
    parser_getThermMode = thermMode_sub.add_parser("get", help="Get thermal control mode state")
    parser_getThermMode.add_argument('-z', '--zone', help='Thermal zone to work with')
    parser_getThermMode.add_argument('-m', '--mode', choices=['custom'], help='The thermal control mode')
    parser_getThermMode.set_defaults(func=getThermalMode)

    #set thermal control mode
    parser_setThermMode = thermMode_sub.add_parser("set", help="Set thermal control mode state")
    parser_setThermMode.add_argument('-z', '--zone', help='Thermal zone to work with')
    parser_setThermMode.add_argument('-m', '--mode', choices=['custom'], help='The thermal control mode')
    parser_setThermMode.add_argument('-s', '--state', choices=['on','off'], help="The values for thermal control mode command")
