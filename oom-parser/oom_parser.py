...
def Usage(script_name):
    sys.stdout.write("Usage: python3 "+ script_name + " -d " + " <data_file>" + " (optional)-p " + "(optional)<PID data>\n")
    ...
    while len(process_res) != 0:
        ...
        if str(rep2).isdigit():
            ...
            if rep1 > rep2:
                ...
                res = "+" + str(change) + "kB"
                if pct_increase > 1:
                    critical_results[max_entry] = [rep1_s, rep2_s, res, "%" + str(pct_increase)[:4]]
        else:
            res = "N/A"
            rep2_s =  rep2
        if len(name) == 0:
...
def gen_mem_makeup():
    ...
    ##mem_file.write(Proc_dict[x].name + ": %" + str(size_pct) + "\n")
    mem_file.close()
...
def get_mem_option():
    gen_mem_sum = input("Would you like to generate memory summary from a memory file?(y/n): ")
