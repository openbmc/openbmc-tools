...
bool ParseI2CTraceLine(std::string line, int& pid, double& timestamp,
                       I2CCmd& cmd, int& i2cid);
                    ...
                    SensorSnapshot** sensor_snapshot)
{
    ...
    for (int i = 0; i < static_cast<int>(services.size()); i++)
    {
        if (i == N - 1 || (i % 100) == 99)
        {
            g_footer_view->SetStatusString(
                fmt::sprintf("2. Getting Creds %d/%d", (i + 1), N));
            DBusTopUpdateFooterView();
        }
    ...
    //        printf("3.2. Call ObjectMapper's GetObject method against the
    //        sensor "
    ...
    int idx = 0;
    N = static_cast<int>(all_obj_paths.size());
    for (const std::string& p : all_obj_paths)
    {
        if (idx % 100 == 99 || idx == N - 1)
        {
            g_footer_view->SetStatusString(fmt::sprintf(
                "3. Examine objects in ObjectMapper %d/%d", idx + 1, N));
            DBusTopUpdateFooterView();
        }
        idx++;
        ...
        if (IsSensorObjectPath(p))
        {
            ...
            if (r < 0)
            {
                //                    printf("Cannot create new method call.
                //                    r=%d, strerror=%s\n", r,
                //                        strerror(-r));
                continue;
            }
            ...
            if (r < 0)
            {
                //                    printf("Could not append a string
                //                    parameter to m\n");
                continue;
            }
            // empty array
            ...
            if (r < 0)
            {
                //                    printf("Could not open a container for
                //                    m\n");
                continue;
            }
            ...
            if (r < 0)
            {
                //                    printf("Could not close container for
                //                    m\n");
                continue;
            }
            r = sd_bus_call(bus, m, 0, &err, &reply);
            if (r < 0)
            {
                //                    printf("Error performing dbus method
                //                    call\n");
            }
            const char* sig = sd_bus_message_get_signature(reply, 0);
            if (!strcmp(sig, "a{sas}"))
            {
                ...
                if (r < 0)
                {
                    //                        printf("Could not enter the level
                    //                        0 array container\n");
                    continue;
                }
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        {
                            (*sensor_snapshot)
                                ->SerSensorVisibleFromObjectMapper(
    ...
    // 3.5: Find all objects under ObjectMapper that implement the
    // xyz.openbmc_project.Association interface busctl call
    ...
    idx = 0;
    N = static_cast<int>(assoc_paths.size());
    for (const std::string& assoc_path : assoc_paths)
    {
        if (idx % 100 == 99 || idx == N - 1)
        {
            g_footer_view->SetStatusString(fmt::sprintf(
                "4. List Associations in ObjectMapper %d/%d", idx + 1, N));
            DBusTopUpdateFooterView();
        }
        ++idx;
        err = SD_BUS_ERROR_NULL;
    ...
    idx = 0;
    N = static_cast<int>(services_and_objects.size());
    while (true)
    {
        if (idx % 100 == 99 || idx == N - 1)
        {
            g_footer_view->SetStatusString(fmt::sprintf(
                "5. Find Association Definitions %d/%d", idx + 1, N));
            DBusTopUpdateFooterView();
        }
        ++idx;
    ...
    g_footer_view->SetStatusString("6. Find Associations of Endpoints");
    DBusTopUpdateFooterView();
         ...
         services_and_objects)
    {
        ...
        {
            g_footer_view->SetStatusString(fmt::sprintf(
                "6. Find Associations of Endpoints %d/%d", idx + 1, N));
            DBusTopUpdateFooterView();
        }
    ...
    N = static_cast<int>(comms.size());
    for (int i = 0; i < N; i++)
    {
        if (i % 100 == 99 || i == N - 1)
        {
            g_footer_view->SetStatusString(
                fmt::sprintf("7. Check HWMon DBus objects %d/%d", i + 1, N));
            DBusTopUpdateFooterView();
        }
    ...
    // This is taking too much time, and not adding much useful information.
                                         ...
                                         const char type, sd_bus_message* m)
{
    ...
    // If Sender, SenderPID or SenderCMD is selected, add an entry to stats2pid_
    // so that we can look up I2C stats later
