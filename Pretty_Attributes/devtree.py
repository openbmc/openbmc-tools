...
def assignValues(content):
    ...
    for i in range(1, len(content)):
        ...
        content[i][2] = content[i][2].strip(
            '"'
        )  # removes double quotes(") if any from the value part of each attribute
        ...
        if content[i][0] == "ATTR_HWAS_STATE":
            ...
            state = int(
                state, 16
            )  # state is a hexadecimal string, converts state into an integer
            for j in range(
                2, 8
            ):  # bits 0,1(7,8 from lsb) are reserved, so checking if other bits are set
                is_set = (
                    state >> j
                )  # right shift j bits so that jth  bit will become lsb
                if (
                    is_set & 1
                ):  # Performing logical & with 1 so that if the lsb is set then the value is true
...
def processFile(file_content):
    ...
    for i in range(len(file_content)):
        ...
        if file_content[i] == []:
            continue
        file_content[i] = splitValues(
            file_content[i]
        )  # Attributes are of the form <name> <type> <val>, they are converted into a list [name,type,val]
        file_content[i] = assignValues(
            file_content[i]
        )  # returns a dictionary of attributes
        if "core-0" in file_content[i]["ATTR_PHYS_DEV_PATH"]:
            file_content[i - 1]["ATTR_ECO_MODE"] = file_content[i][
                "ATTR_ECO_MODE"
            ]
            file_content[i - 1]["ATTR_ECO_MODE_DATATYPE"] = file_content[i][
                "ATTR_ECO_MODE_DATATYPE"
            ]
    ...
    for i in range(len(file_content)):
        if len(file_content[i]) and option == 0:
            if len(sys.argv) == 1:
                ...
                printContent(
                    file_content[i]
                )  # prints whole content in normal text format
                print("\n")
            else:
                if sys.argv[1] == "-j":
                    if len(sys.argv) == 2:
                        ...
                        cont = getLimitedAttributes(
                            file_content[i], attrs
                        )  # gets a list of attributes specified by the user
                        key = file_content[i]["ATTR_PHYS_DEV_PATH"].split(":")
                        key = key[-1].split("/")
                        k = k_v
                        j = 0
                    ...
                    cont = getLimitedAttributes(
                        file_content[i], attrs
                    )  # gets a list of attributes specified by user
                    print()
                    tar = "target : " + file_content[i]["TARGET"]
                    print(tar.center(50, "*"))
                    printContent(
                        cont
                    )  # prints only the specified attributes in normal text format
            ...
            if len(sys.argv) == 3:
                if re.search(
                    "^" + sys.argv[2],
                    file_content[i]["ATTR_PHYS_DEV_PATH"].split("/")[-1],
                ):
                ...
                if re.search(
                    ...
                    for x in sys.argv[3:]:
                        x = x.split("=")
                        if (
                            file_content[i][x[0].upper()].upper()
                            != x[1].upper()
                        ):
                            flag = 1
                    if flag == 0:
    ...
    if "bmc" not in host_name.stdout.decode("utf-8"):
        for i in range(len(file_content)):
            ...
            if tar == target:
                ...
                if "-notdev" in sys.argv and "dimm" in target:
                    j = i - 1
                    while j >= 0:
                        if len(file_content[j]) == 0:
                            j -= 1
                            continue
                        if "omi" not in file_content[j]["TARGET"] and (
                            "adc" in file_content[j]["TARGET"]
                            or "gpio_expander" in file_content[j]["TARGET"]
                            or "ocmb" in file_content[j]["TARGET"]
                            or "mem_port" in file_content[j]["TARGET"]
                        ):
                            cmd = (
                                "sudo -i attributes translate"
                                + " "
                                + file_content[j]["TARGET"]
                            )
                            ...
                            if "/" not in x[0]:
                                x[0], x[-1] = x[-1], x[0]
                            path_list[x[-1]] = x[0]
                        if "omi" in file_content[j]["TARGET"]:
                            break
                        j -= 1
                    j = i + 1
                    while j < len(file_content):
                        if len(file_content[j]) == 0:
                            j += 1
                            continue
                        if "pmic" in file_content[j]["TARGET"]:
                            cmd = (
                                "sudo -i attributes translate"
                                + " "
                                + file_content[j]["TARGET"]
...
def getParent(file_content, target):
    path_list = OrderedDict()
    if "bmc" not in host_name.stdout.decode("utf-8"):
        if "-notdev" in sys.argv and (
            "pmic" in target
            or "adc" in target
            or "gpio_expander" in target
            or "ocmb" in target
            or "mem_port" in target
        ):
            for i in range(len(file_content)):
                if file_content[i] == []:
                    continue
                ...
                if tar == target:
                    if "pmic" in target:
                        ...
                        while j >= 0:
                            if len(file_content[j]) == 0:
                                j -= 1
                                continue
                            if "dimm" not in file_content[j]["TARGET"]:
                                j -= 1
                            else:
                                break
                    else:
                    ...
                    ssh_client.connect(
                        host, port=22, username=uname, password=pwd
                    )
                    cmd = (
                        "sudo -i attributes translate"
                        + " "
...
def usage():
    ...
    cwd = os.getcwd()
    print(sys.argv)
    cwd += "/" + sys.argv[0].strip("devtree.py")
    cwd += "help_usage.txt"
...
if len(sys.argv) < 2 or (
    ...
    and (
        sys.argv[1]
        not in ["-h", "-t", "-p", "-l", "-n", "-f", "-parent", "-child"]
    )
):  # Goes inside, if there is no option or the option is -j
