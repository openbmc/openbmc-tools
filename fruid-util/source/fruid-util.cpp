...
#include "../includes/fruid-util.hpp"
...
int main( int cArguments, char** apszArguments ) {
   ...
   try {
      ...
      bpo::store(bpo::command_line_parser(cArguments, apszArguments)
                     .options(od)
                     .positional(p)
                     .run(),
                 vm);
      ...
      if( vm.count("version")) {
         std::cout << "FRUID-UTIL version 0.1 ("
                   << __TIMESTAMP__ << ")"
                   << std::endl;
         return EXITCODE_SUCCESS;
         }
      ...
      std::cerr << "?Exception while parsing command line:"
      << se.what()
      << std::endl;
      return EXITCODE_FAILURE;}
   catch( ... ) {
      std::cerr << "?Unhandled exception while parsing command line"
                << std::endl;
      return EXITCODE_FAILURE;}      
   ...
   try {
      ...
      std::cerr << "?Exception while retrieving FRUDevice.GetManagedObjects" 
                << se.what()
                << std::endl;
      return EXITCODE_CANT_GET_MANAGED_OBJECTS;}
   catch( ... ) { 
      std::cerr << "?Unhandled exception while retrieving FRUDevice.GetManagedObjects"
                << std::endl;
      return EXITCODE_CANT_GET_MANAGED_OBJECTS;}
   ...
   try {
            ...
            for (const auto& j : i.second)
            {
                for (const auto& k : j.second)
                {
                    ...
                    switch (k.second.index())
                    {
                        case 0:
                        {
                            std::string x = std::get<std::string>(k.second);
                            std::cout << x << std::endl;
                        }
                        break;
                        case 6:
                        {
                            uint32_t u = std::get<uint32_t>(k.second);
                            std::cout << u << std::endl;
                        }
                        break;
                        default:
      ...
      std::cerr << "?Exception while displaying results:" 
                << se.what() 
                << std::endl;
                return EXITCODE_FAILURE;}
   catch( ... ) {
      std::cerr << "?Unhandled exception while displaying results" 
                << std::endl;
                return EXITCODE_FAILURE;}
   }  