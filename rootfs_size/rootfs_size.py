...
parser.add_argument("-b", "--build_dir",
                    dest="BUILD_DIR",
                    default="/home/ed/openbmcup",
                    help="Build directory path.")

parser.add_argument("-s", "--squashfs_file",
                    dest="SQUASHFS_FILE",
                    default="/build/deploy/images/qemux86-64/obmc-phosphor-image-qemux86-64-20210218021842.rootfs.squashfs-xz",
                    help="Squashfs file.")
...
def get_unsquash_results(filepath):
    with tempfile.TemporaryDirectory() as newsquashfsroot:
        ...
        shutil.copytree(squashfsdir, input_path, symlinks=True, ignore_dangling_symlinks=True)
...
with tempfile.TemporaryDirectory() as tempsquashfsdir:
        ...
        for name in files + dirs:
            ...
            if not os.path.islink(filepath):
                if getsize(filepath) > FILE_SIZE_LIMIT:
                  files_to_test.append(os.path.relpath(filepath, squashfsdir))
    ...
    with Pool(threads) as p:
      for i, res in enumerate(p.imap_unordered(get_unsquash_results, files_to_test)):
        results.append(res)
        sys.stderr.write('\rdone {:.1f}%'.format(100* (i/len(files_to_test))))
